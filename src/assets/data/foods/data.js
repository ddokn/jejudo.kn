export default [	
	{
		restaurant : '맛나식당',
		summary : '한 번 가면 무조건 밥 두 공기 뚝딱, 고등어/갈치 반반으로만 주문 가능',
		address: '제주 서귀포시 성산읍 동류암로 41',
		category : '성산·우도',
		last_date : '2021-03-04',
		color : '',
		count: '3',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/MZ7Gt7G0/2.jpg',
			'https://i.postimg.cc/dVtDhhw2/1.jpg',
			'https://i.postimg.cc/d3jt1YfN/3.jpg',
		],
		accompany : [
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
			{
				name: '이주승',
				avatar: '',
			},
			{
				name: '강진현',
				avatar: '',
			}
		]
	},
	{
		restaurant : '제주 미담',
		summary : '소문에 비해 맛은 평범. 숙소와 가깝다면 한 번쯤 가볼만함',
		address: '제주 제주시 관덕로11길 8-1',
		category : '제주시',
		last_date : '2021-01-30',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/PJgcBpyB/7031.jpg',
			'https://i.postimg.cc/wBx6WZ7D/7032.jpg',
			'https://i.postimg.cc/6pfWft59/7062.jpg',
		],
		accompany : [
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
			{
				name: '이주승',
				avatar: '',
			},
			{
				name: '강진현',
				avatar: '',
			}
		]
	},
	{
		restaurant : '옹포리(옹포밥집)',
		summary : '분위기는 괜츈. 가끔 생각나는 맛',
		address: '제주 제주시 한림읍 한림로 497 1층 옹포리',
		category : '한림·협재',
		last_date : '2021-03-02',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/jqc4mJgN/7646.jpg',
			'https://i.postimg.cc/7hK12pGX/IMG-3193.jpg',
		],
		accompany : [
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
			{
				name: '한효정',
				avatar: '',
			},
		]
	},
	{
		restaurant : '관덕정 분식',
		summary : '분식치고는 가격대가 비싼 편으로 맛은 쏘쏘',
		address: '제주 제주시 관덕로8길 7-9',
		category : '제주시',
		last_date : '2021-03-03',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/L5PYHwkn/7687.jpg',
			'https://i.postimg.cc/MTMXDNFd/7688.jpg',
			'https://i.postimg.cc/d00h7LmM/7689.jpg',
			'https://i.postimg.cc/852cnw8D/7690.jpg',
		],
		accompany : [
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
			{
				name: '한효정',
				avatar: '',
			},
		]
	},
	{
		restaurant : '골막식당(골막국수)',
		summary : '택시 기사님이 추천해주신 곳. 일반 국수보다 면이 두꺼운 편이로 국물, 김치가 기가막힘',
		address: '제주 제주시 천수로 12',
		category : '제주시',
		last_date : '2022-03-05',
		color : '',
		count: '3',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/26mXFNyf/7718.jpg',
			'https://i.postimg.cc/g03TZ2YD/IMG-3379.jpg',
			'https://i.postimg.cc/YCGVtxZT/7717.jpg',
		],
		accompany : [
			{
				name: '익명의 택시기사님',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
			{
				name: '인치현',
				avatar: '',
			},
			{
				name: '장혜진',
				avatar: '',
			},
			{
				name: '김민정',
				avatar: '',
			},
		]
	},
	{
		restaurant : '은희네 해장국',
		summary : '다른거 말고 무조건 순두부 찌개로 두 번 머거',
		address: '제주 서귀포시 중문관광로 348 나동 1층',
		category : '중문',
		last_date : '2021-03-23',
		color : '',
		count: '1',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/6qQ7ZTN4/1.jpg',
			'https://i.postimg.cc/qMzhMtL9/3.jpg',
			'https://i.postimg.cc/NGN2c816/2.jpg',

		],
		accompany : [
			{
				name: '허진강',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
		]
	},
	{
		restaurant : '고성장터국밥',
		summary : '저렴한 가격, 비린내 전혀 없는 완벽한 맛',
		address: '제주 서귀포시 성산읍 고성동서로45번길 19',
		category : '성산·우도',
		last_date : '2021-03-24',
		color : '',
		count: '2',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/x1PBqdtS/IMG-1151.jpg',
			'https://i.postimg.cc/9z5YYGXN/IMG-1148.jpg',
			'https://i.postimg.cc/6Q5gZNm4/IMG-1149.jpg',

		],
		accompany : [
			{
				name: '허진강',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
		]
	},
	{
		restaurant : '순희밥상',
		summary : '제주식 정갈한 집 밥 느낌. 조용한 동네. 깔끔한 내부',
		address: '제주 제주시 구좌읍 종달로5길 38',
		category : '구좌·월정',
		last_date : '2021-03-24',
		color : '',
		count: '1',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/gkbMR6S6/IMG-1238.jpg',
			'https://i.postimg.cc/Xv676LQY/IMG-1237.jpg',
			'https://i.postimg.cc/g2N4ftYv/IMG-1241.jpg',

		],
		accompany : [
			{
				name: '허진강',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
		]
	},
	{
		restaurant : '평화반점',
		summary : '가지마.... 제발........ 명령임..',
		address: '제주특별자치도 제주시 중앙로 486 1층',
		category : '제주시',
		last_date : '2022-08-14',
		color : '',
		count: '4',
		point: '1',
		thumbnail: [
			'https://i.postimg.cc/rwX3jPfg/B697-A8-E9-19-CE-4294-8-A5-C-A9808-E2-FA0-A1.jpg',
			'https://i.postimg.cc/9Xvc31xw/IMG-2603.jpg',
		],
		accompany : [
			{
				name: '김인규',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '신미향',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
			},
		]
	},
	{
		restaurant : '짱구 분식',
		summary : '처음으로 모닥치기를 먹은 곳, 먹을만 함',
		address: '제주특별자치도 서귀포시 중동로48번길 3',
		category : '서귀포',
		last_date : '2021-10-04',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/T2kWr83M/51-ACB434-5753-41-E8-9-E49-90-C64-FC760-DE.jpg',
			'https://i.postimg.cc/VLd0g7JS/79-D72-FA7-598-D-4292-98-E9-A569-BECBE701.jpg',
		],
		accompany : [
			{
        name: '김도근',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
		]
	},
	{
		restaurant : '원담',
		summary : '고등어회에 거부감이 있는 사람도 무리 없이 먹을 수 있는 곳',
		address: '제주특별자치도 제주시 동광로1길 13 1층',
		category : '제주시',
		last_date : '2022-11-25',
		color : '',
		count: '3',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/zGSVx4Fk/IMG-2781.jpg',
			'https://i.postimg.cc/cJF6SYjg/IMG-2782.jpg',
		],
		accompany : [
			{
        name: '이수진',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
			{
        name: '장혜진',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
      {
        name: '김도근',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
		]
	},
	{
		restaurant : '바다를 본 돼지',
		summary : '흑돼지, 오션 뷰 맛집, 요즘은 유명해졌는지 웨이팅이 꽤나 긴 편',
		address: '제주특별자치도 서귀포시 칠십리로 123 2층',
		category : '서귀포',
		last_date : '2022-09-30',
		color : '',
		count: '5',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/fTVps023/IMG-3009.jpg',
			'https://i.postimg.cc/NjTn9vwB/IMG-3011.jpg',
			'https://i.postimg.cc/pdH6Kc28/IMG-3014.jpg',
		],
		accompany : [
			{
        name: '김민희',
        avatar: 'https://i.postimg.cc/4yGnsn32/IMG-5041.jpg',
      },
			{
        name: '이주승',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
			{
        name: '강진현',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
			{
        name: '루덴스',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
			{
        name: '김진선',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
      {
        name: '김도근',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
		]
	},
	{
		restaurant : '노라바 해물라면',
		summary : '두 번째 방문 때 달라진 맛에 실망. 애월해안도로 근처라 뷰는 만점',
		address: '제주 제주시 애월읍 구엄길 100',
		category : '애월',
		last_date : '2022-02-26',
		color : '',
		count: '2',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/t4WVL2FB/IMG-3266.jpg',
			'https://i.postimg.cc/6pJX10W6/IMG-1772.jpg',
		],
		accompany : [
			{
        name: '유별남',
        avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
      },
			{
        name: '김도근',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
			{
        name: '레오',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
			{
        name: '정은봉',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
			{
        name: '스노우',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      }
		]
	},
	{
		restaurant : '삼남매 식당',
		summary : '친구 왈: "여태까지 제주에서 먹은 음식 중 가장 맛있었다"',
		address : '제주특별자치도 제주시 인다9길 18',
		category : '제주시',
		last_date : '2022-09-26',
		color : '',
		count: '3',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/C50bW8kd/IMG-3309.jpg',
			'https://i.postimg.cc/c17RfDG3/IMG-3310.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			},
					{
				name: '정은봉',
				avatar: '',
			},
					{
				name: '송선용',
				avatar: '',
			},
					{
				name: '이성령',
				avatar: '',
			},
			{
				name: '엄장원',
				avatar: '',
			},
			{
				name: '김민희',
				avatar: '',
			}
		]
	},
	{
		restaurant : '이로운 술상',
		summary : '해산물이 상당히 신선하고, 분위기 또한 좋아서 술이 술술',
		address : '제주 제주시 조천읍 함덕16길 44',
		category : '조천·함덕',
		last_date : '2022-04-18',
		color : '',
		count: '1',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/XYcHYwnK/IMG-3509.jpg',
			'https://i.postimg.cc/YCkRZhQv/IMG-3510.jpg',
			'https://i.postimg.cc/YqQ8d1Lc/IMG-3512.jpg',
		],
		accompany : [
			{
        name: '김도근',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
			{
        name: '김지영',
        avatar: '',
      }
		]
	},
	{
		restaurant : '차돌집',
		summary : '사람만 많지 않으면 더 자주 갔을 법한 차돌 찐찐 맛집',
		address : '제주 제주시 신대로10길 29',
		category : '제주시',
		last_date : '2022-04-21',
		color : '',
		count: '4',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/jd5W3TFz/IMG-3517.jpg',
			'https://i.postimg.cc/KYXRYMvN/IMG-3518.jpg',
			'https://i.postimg.cc/vZx4MzMT/IMG-3523.jpg',
			'https://i.postimg.cc/fbxkSRVh/IMG-3524.jpg',
			'https://i.postimg.cc/CLQRW239/IMG-3525.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
        name: '김도근',
        avatar: 'https://i.postimg.cc/h45XzZ60/IMG-5235-3.jpg',
      },
			{
        name: '고지원',
        avatar: '',
      },
			{
        name: '정은봉',
        avatar: 'https://i.postimg.cc/J4JyC4gw/IMG-3173.jpg',
      },
			{
        name: '김민희',
        avatar: '',
      }
		]
	},
	{
		restaurant : '벵디',
		summary : '돌문어 덮밥 강추강추',
		address : '제주특별자치도 제주시 구좌읍 해맞이해안로 1108',
		category : '구좌·월정',
		last_date : '2022-04-23',
		color : '',
		count: '1',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/FzqhnMYZ/IMG-3534.jpg',
			'https://i.postimg.cc/pV9rKChZ/IMG-3533.jpg',
			'https://i.postimg.cc/nV6z2mqt/IMG-3530.jpg',
			'https://i.postimg.cc/Th8ZbfyZ/IMG-3527.jpg',
			'https://i.postimg.cc/KvBYHY53/IMG-3531.jpg',

		],
		accompany : [
			{
				name: '정은봉',
				avatar: 'https://i.postimg.cc/J4JyC4gw/IMG-3173.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			}
		]
	},
	{
		restaurant : '닻',
		summary : '딱새우 맛집. 2층보단 1층에 앉는걸 추천',
		address : '제주특별자치도 제주시 애월읍 가문동길 41-2 1층',
		category : '애월',
		last_date : '2022-04-23',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/YCXBncTm/IMG-3584.jpg',
			'https://i.postimg.cc/Xvz3JM80/IMG-3585.jpg',
			'https://i.postimg.cc/XvJ44N98/IMG-3587.jpg',
			'https://i.postimg.cc/tTnbnrSn/IMG-3589.jpg',
		],
		accompany : [
			{
				name: '정은봉',
				avatar: 'https://i.postimg.cc/J4JyC4gw/IMG-3173.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '고민수',
				avatar: '',
			}
		]
	},
	{
		restaurant : '홍보석',
		summary : '면이 일반적인 중식 면보단 얇은 편, 비빔 짬뽕 추천',
		address : '제주 제주시 비월길 5',
		category : '제주시',
		last_date : '2022-05-20',
		color : '',
		count: '2',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/JhV7cCXc/IMG-3768.jpg',
			'https://i.postimg.cc/mDVbsc58/IMG-3770.jpg',
			'https://i.postimg.cc/DyR7n3xk/IMG-3771.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},{
				name: '김도근',
				avatar: '',
			}
		]
	},
	{
		restaurant : '인디언썸머',
		summary : '분위기, 가격, 맛 다 평균 이상. 애월 필수 코스',
		address : '제주특별자치도 제주시 애월읍 애월로4길 9 1층',
		category : '애월',
		last_date : '2022-05-20',
		color : '',
		count: '1',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/d31JtsMv/IMG-3829.jpg',
			'https://i.postimg.cc/ZY7Tsd3q/IMG-3831.jpg',
			'https://i.postimg.cc/brBpZ78n/IMG-3832.jpg',
		],
		accompany : [
			{
				name: '유별남',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '김해니',
				avatar: '',
			}
		]
	},
	{
		restaurant : '성산연화',
		summary : '직원분들이 친절, 와인을 좋아한다면 한 번쯤 갈만한 곳',
		address : '제주특별자치도 서귀포시 성산읍 성산중앙로 22',
		category : '성산·우도',
		last_date : '2022-06-11',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/Y9tpZNbv/IMG-3991.jpg',
			'https://i.postimg.cc/TYKTz3qJ/IMG-3995.jpg',
			'https://i.postimg.cc/8zWNCXYF/IMG-3997.jpg',
		],
		accompany : [
			{
				name: '유별남',
				avatar: 'https://i.postimg.cc/L8H346WW/default-profile.png',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '김해니',
				avatar: '',
			}
		]
	},
	{
		restaurant : '쇠소깍횟집',
		summary : '제주시 사는 사람치고 여기 모르는 사람 못 봄',
		address : '제주특별자치도 서귀포시 성산읍 성산중앙로 22',
		category : '제주시',
		last_date : '2022-06-22',
		color : '',
		count: '2',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/KY2F3rX0/IMG-4093.jpg',
			'https://i.postimg.cc/g0HpVXmm/IMG-4096.jpg',
			'https://i.postimg.cc/C5zVpcDx/IMG-4097.jpg',
			'https://i.postimg.cc/zBd1QsyQ/IMG-4098.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '정은봉',
				avatar: '',
			},
			{
				name: '인치현',
				avatar: '',
			},
			{
				name: '김민정',
				avatar: '',
			}
		]
	},
	{
		restaurant : '뱃사공',
		summary : '깔끔. 비린내 없음. 가끔 갈만함',
		address : '제주 제주시 진동로 36',
		category : '제주시',
		last_date : '2022-06-30',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/FRF4DrHc/IMG-4115.jpg',
			'https://i.postimg.cc/WzCcN066/IMG-4116.jpg',
			'https://i.postimg.cc/66mXfNJ2/IMG-4118.jpg',
		],
		accompany : [
			{
				name: '정은봉',
				avatar: 'https://i.postimg.cc/J4JyC4gw/IMG-3173.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '고민수',
				avatar: '',
			},
		]
	},
	{
		restaurant : '제라한보쌈',
		summary : '차마 제주까지 놀러온 사람한테 보쌈집 추천해주긴 좀 그렇지만 그럼에도 꼭 갔으면 하는 곳',
		address : '제주 제주시 정존1길 33',
		category : '제주시',
		last_date : '2022-08-02',
		color : '',
		count: '3',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/L6gMy02y/IMG-4449.jpg',
			'https://i.postimg.cc/kgxPC6VK/IMG-4450.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '고지원',
				avatar: '',
			}
		]
	},
	{
		restaurant : '이자카야 상상',
		summary : '분위기 좋고 맛 좋고 상상 이상',
		address : '제주 제주시 조천읍 신북로 487 상상 함덕점',
		category : '조천·함덕',
		last_date : '2022-08-13',
		color : '',
		count: '1',
		point: '4',
		thumbnail: [
			'https://i.postimg.cc/6psxFLQB/IMG-4510.jpg',
			'https://i.postimg.cc/xdnD1fH7/IMG-4511.jpg',
			'https://i.postimg.cc/KYfXK6DY/IMG-4512.jpg',
			'https://i.postimg.cc/G2mWvXXx/IMG-4513.jpg',
		],
		accompany : [
			{
				name: '정은봉',
				avatar: 'https://i.postimg.cc/J4JyC4gw/IMG-3173.jpg',
			},
			{
				name: '고민수',
				avatar: '',
			},
			{
				name: '최영근',
				avatar: '',
			},
			{
				name: '최효진',
				avatar: '',
			},
			{
				name: '김도근',
				avatar: '',
			}
		]
	},
	{
		restaurant : '대굴대굴',
		summary : '겨울철(10월 ~ 3월)에만 오픈. 올해는 너무 많이가서 질려버렸지만 내년에 또 갈래',
		category : '제주시',
		address: '제주특별자치도 제주시 오복2길 17',
		last_date : '2022-11-11',
		color : '',
		count: '4',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/8cFx0jQF/IMG-5053.jpg',
			'https://i.postimg.cc/HnfFDdd6/IMG-5062.jpg',
			'https://i.postimg.cc/7PcFTM0S/IMG-5065.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김진선',
				avatar: '',
			},
			{
				name: '최영근',
				avatar: '',
			},
			{
				name: '최효진',
				avatar: '',
			},
			{
				name: '김도근',
				avatar: '',
			}
		]
	},
	{
		restaurant : '태광식당',
		summary : '한지 주물럭과 돼지 주물럭을 같이 시켜서 먹는 걸 추천. 맵지 않아 아이들과 가도 좋을 듯',
		address: '제주 제주시 탑동로 142',
		category : '제주시',
		last_date : '2023-01-04',
		color : '',
		count: '1',
		point: '3',
		thumbnail: [
			'https://i.postimg.cc/3xQzt8LR/IMG-5200.jpg',
			'https://i.postimg.cc/d0qpKWcc/IMG-5201.jpg',
			'https://i.postimg.cc/t4dwfKfz/IMG-5202.jpg',
		],
		accompany : [
			{
				name: '임효현',
				avatar: 'https://i.postimg.cc/Bv6Hr0cD/2023-01-13-4-22-47.png',
			},
			{
				name: '김민희',
				avatar: '',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '박정권',
				avatar: '',
			}
		]
	},
	{
		restaurant : '만금',
		summary : '잔말말고 제주 어디에있던 여긴 꼭 가보길. 안가면 손해임',
		address: '제주 제주시 도남로15길 11',
		category : '제주시',
		last_date : '2022-10-04',
		color : '',
		count: '5',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/vmXzbmpH/IMG-3935.jpg',
			'https://i.postimg.cc/G25xT5LW/IMG-4923.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '한효정',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김도근',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '인치현',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김민정',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '장혜진',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '민지홍',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '안현진',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			}
		]
	},
	{
		restaurant : '고서방 짬뽕',
		summary : '아주아주 애정하는 곳. 재료소진이 매우 빠른 편이라 오픈시간에 맞춰 가는걸 추천',
		address: '제주 제주시 인다5길 11-5',
		category : '제주시',
		last_date : '2023-01-07',
		color : '',
		count: '7',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/FsZDLn21/2.jpg',
			'https://i.postimg.cc/hv8MMCvy/1.jpg',
			'https://i.postimg.cc/JzzKc5Lx/3.jpg',
		],
		accompany : [
			{
				name: '고민수',
				avatar: 'https://i.postimg.cc/h4XyHCnR/Kakao-Talk-Photo-2023-01-12-15-25-05.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			}
		]
	},
	{
		restaurant : '오늘은 막창 먹자',
		summary : '잡내 없고 구워서 나오는 막창이 아주아주 쫄깃. 최근 가장 자주 찾는 곳',
		address: '제주 제주시 동고산로 11',
		category : '제주시',
		last_date : '2022-12-15',
		color : '',
		count: '4',
		point: '5',
		thumbnail: [
			'https://i.postimg.cc/T15qVJhp/IMG-4972.jpg',
			'https://i.postimg.cc/z3mw0HRp/IMG-4973.jpg',
			'https://i.postimg.cc/hjj0jBB8/IMG-4974.jpg',
		],
		accompany : [
			{
				name: '정은봉',
				avatar: 'https://i.postimg.cc/J4JyC4gw/IMG-3173.jpg',
			},
			{
				name: '김도근',
				avatar: '',
			},
			{
				name: '김민희',
				avatar: '',
			},
			{
				name: '고민수',
				avatar: '',
			},
			{
				name: '최영근',
				avatar: '',
			}
		]
	}
]